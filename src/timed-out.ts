...
export class TimedOut {
    static applyToRequest(req: TimedOut.MyClientRequest, time: number): TimedOut.MyClientRequest {
        ...
        var host: string = (req as any)._headers ? (req as any)._headers.host : '';
        var remoteAddress: string = null;

        if (delays.connect !== undefined) {
            (req as any).timeoutTimer = setTimeout(function timeoutHandler() {
                req.abort();
        ...
        req.on('socket', function assign(socket: net.Socket) {
            ...
            if (!(socket.connecting || (socket as any)._connecting)) {
                connect();
                return;
            }
